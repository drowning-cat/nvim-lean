...
vim.keymap.set("o", "C", "gc", { desc = "Comment textobject" })
...
function _G.after_restart(orig_session)
  vim.cmd.source("Restart.vim")
  vim.fs.rm("Restart.vim", { force = true })
...
vim.keymap.set("n", "<Leader>R", function()
  ...
  vim.cmd.mksession({ "Restart.vim", bang = true })
...
-- -- Detect root
-- -- NOTE: See `MiniSessions`, `MiniVisits`
...
vim.keymap.set("n", "<Leader>.F", function() MiniFiles.open(find_root()) end, { desc = "Open files (project root)" })
...
local get_ts_highlights = function(source, lang, parse_range, query_range)
  ...
  query_range = query_range or { nil, nil, nil, nil }
  ---@class ExtmarkExtra : vim.api.keyset.set_extmark
  ---@field row? number
  ---@field col? number
  -- WARN: Key order is not guaranteed: { [15] = { ... }, [1] = { ... } }
  local ret = {} ---@type table<number, ExtmarkExtra[]>
  local parser ---@type vim.treesitter.LanguageTree?
  if type(source) == "number" then
    parser = vim.treesitter.get_parser(source, lang, { error = false })
  end
  if type(source) == "string" then
    parser = vim.treesitter.get_string_parser(source, lang, { error = false })
  end
  if not parser then
    error("Unable to get a parser")
  end
  ...
  parser:for_each_tree(function(tree, ltree)
    ...
    for capture, node, metadata in
      query:iter_captures(
        ...
        query_range[1],
        query_range[3],
        { start_col = query_range[2], end_col = query_range[4] }
      )
    do
      ...
      if name ~= "spell" then
        ...
        for row = range[1] + 1, range[3] + 1 do
          ...
          ret[row] = ret[row] or {}
          -- HACK: Increase the priority each time the same column appears
          local col = first and range[2] or 0
          local prior = tonumber(metadata.priority or metadata[capture] and metadata[capture].priority) or 100
          local prior_id = vim.inspect({ row, col })
          prior = priorities[prior_id] and priorities[prior_id] + 1 or prior
          priorities[prior_id] = prior
...
local get_lang = function(ft)
  ...
  if not lang or not vim.api.nvim_get_runtime_file("parser/" .. lang .. ".so", false) then
    error(string.format("No parser for ft '%s'", ft))
  end
  if not vim.api.nvim_get_runtime_file("queries/" .. lang .. "/highlights.scm", false) then
    error(string.format("No highlights for lang '%s' (%s)", lang, ft))
  end
  return lang
end
-- local get_text_highlights = function(text, ft)
--   text = type(text) == "table" and table.concat(text, "\n") or text
--   local lang = assert(get_lang(ft))
--   return get_ts_highlights(text, lang)
-- end
local get_text_patched_highlights = function(text, ft)
  text = type(text) == "table" and table.concat(text, "\n") or text
  local lang = assert(get_lang(ft))
  -- HACK: Lua tresitter does not highlight incomplete keywords
  if lang == "lua" then
    local keywords_prepend = { ["end"] = "do", ["until"] = "repeat" }
    local keywords_append = { ["for"] = "end", ["while"] = "end", ["if"] = "end", ["repeat"] = "until" }
    local lines = vim.split(text, "\n")
    local first_line, last_line = lines[1], lines[#lines]
    local before, after = {}, {}
    for _, word in ipairs(vim.split(first_line, "%s+")) do
      local prepend = keywords_prepend[word]
      if prepend then
        table.insert(before, prepend)
      end
    end
    for _, word in ipairs(vim.split(last_line, "%s+")) do
      local append = keywords_append[word]
      if append then
        table.insert(after, append)
      end
    end
    local from, to
    if not vim.tbl_isempty(before) then
      from = #before
      text = table.concat(before, "\n") .. "\n" .. text
    end
    if not vim.tbl_isempty(after) then
      to = math.max(1, #{ string.find(text, "\n") })
      text = text .. "\n" .. table.concat(after, "\n")
    end
    return get_ts_highlights(text, lang, true, { from, nil, to, nil })
  end
  return get_ts_highlights(text, lang)
end
local get_buf_highlights = function(buf, with_extmarks, ln_range)
  ...
  ln_range = vim.tbl_extend("keep", ln_range or {}, { -- [line1, line2) 0-based
    [1] = 0,
    [2] = vim.api.nvim_buf_line_count(buf),
  })
  local lang = assert(get_lang(vim.bo[buf].ft))
  local range4 = { ln_range[1], nil, ln_range[2], nil }
  local ret = get_ts_highlights(buf, lang, false, range4)
  if with_extmarks then
    local start, _end = { ln_range[1], 0 }, { ln_range[2] - 1, -1 }
    local extmarks = vim.api.nvim_buf_get_extmarks(buf, -1, start, _end, { details = true })
...
local hls_to_list = function(hls)
  hls = vim.deepcopy(hls or {}, true)
  local keys = {}
  for row in pairs(hls) do
    table.insert(keys, row)
  end
  table.sort(keys)
  local ret = {}
  for _, key in ipairs(keys) do
    table.insert(ret, hls[key])
  end
  return ret, keys[1]
end
local buf_set_highlights = function(buf, ns, row_start, col_offset, hls)
  row_start, col_offset = row_start or 0, col_offset or 0
  local hls_list = hls_to_list(hls)
  for i, line_hls in ipairs(hls_list) do
    local row = row_start + i - 1
    for _, hl in ipairs(line_hls) do
      local col = hl.col + col_offset
      hl.row = nil
      hl.col = nil
      hl.end_col = hl.end_col and (hl.end_col + col_offset) or nil
      local ok, err = pcall(vim.api.nvim_buf_set_extmark, buf, ns, row, col, hl)
      if not ok then
        vim.notify(string.format("Set extmark buf=%s|row=%s|col=%s: %s", buf, row, col, err), vim.log.levels.WARN)
      end
    end
  end
end
local minipick_cache = {}
local minipick_use_cache = function(id, func)
...
local default_color_lines = function(buf, data, get_hls)
  vim.api.nvim_buf_clear_namespace(buf, minipick_hl_ns, 0, -1)
  local lines = vim.api.nvim_buf_get_lines(buf, 0, -1, true)
  for i = 1, #data do
    local item, line = data[i], lines[i]
    local start, line_hls = get_hls(item, line)
    if start and line_hls then
      local row_0, col_0 = i - 1, start - 1
      buf_set_highlights(buf, minipick_hl_ns, row_0, col_0, line_hls)
    end
  end
end
-- -- -- Overrides (+colors)
MiniPick.registry.buf_lines = function(local_opts, opts)
  ...
  local show = function(buf, data, query)
    ...
    default_color_lines(buf, data, function(item, line)
      local item_buf, row = item.bufnr, item.lnum
      local id = vim.inspect({ item_buf, row })
      return minipick_use_cache(id, function()
  ...
  return MiniExtra.pickers.buf_lines(
    local_opts, --
    vim.tbl_deep_extend("keep", opts or {}, { source = { show = show } })
  )
end
MiniPick.registry.git_hunks = function(local_opts, opts)
  ...
  local show = function(buf, data, query)
    ...
    default_color_lines(buf, data, function(item, line)
      ...
      return start,
        minipick_use_cache(id, function()
          local file = item.path
          local ft = vim.filetype.match({ filename = file, contents = { code } })
          local ok, hls = pcall(get_text_patched_highlights, code, ft)
          return ok and hls or nil
        end)
    end)
  end
...
local grep_show = function(buf, data, query, opts)
  ...
  default_color_lines(buf, data, function(item, line)
    ...
    return minipick_use_cache(id, function()
      ...
      local ft = vim.filetype.match({ filename = file, contents = { code } })
      local ok, hls = pcall(get_text_patched_highlights, code, ft)
      return start, ok and hls or nil
    end)
  end)
end
...
MiniPick.registry.grep_todo = function(local_opts, opts)
  ...
  return MiniPick.registry.grep( --
    vim.tbl_deep_extend("keep", local_opts or {}, { pattern = pattern }),
    opts
  )
end
MiniPick.registry.hipatterns = function(local_opts, opts)
  local show = function(buf, data, query)
    ...
    -- NOTE: Place after `default_color_lines` (clears `minipick_hl_ns`)
    for i = 1, #data do
      local item = data[i]
  ...
  return MiniExtra.pickers.hipatterns(
    local_opts, --
    vim.tbl_deep_extend("keep", opts or {}, { source = { show = show } })
  )
end
...
local format = function(local_opts)
  local_opts = local_opts or {}
  local range = local_opts.range or {}
  local async = local_opts.async or false
...
_G.Formatexpr = function(opts)
  ...
  if format({ range = { line1, line2 } }) then
  else
...
vim.api.nvim_create_autocmd("BufWritePre", {
  ...
  callback = function()
    ...
    local opts_list = { vim.b.format_on_save, vim.g.format_on_save, if_unset }
    local enabled = vim.iter(opts_list):find(function(val)
      return val ~= nil
    end)
    if enabled then
