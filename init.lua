...
vim.cmd.rshada()
if pcall(vim.cmd.colorscheme, vim.g.COLORS_NAME or vim.g.default_colors) then
...
local MiniPick = require("mini.pick")
MiniPick.run_action = function(action_or_keys)
  local run = MiniPick.get_picker_opts().mappings[action_or_keys] or action_or_keys
  if type(run) == "function" then
    run()
  end
  if type(run) == "string" then
    vim.api.nvim_feedkeys(vim.keycode(run), "n", false)
  end
end
MiniPick.registry.colorschemes = function(local_opts, opts)
  ...
  vim.api.nvim_create_autocmd("User", {
    pattern = { "MiniPickStart", "MiniPickMatch" },
    group = aug,
    callback = function()
      preview()
    end,
  ...
  local move_remap = function(lhs, rhs)
    return {
      char = lhs,
      func = function()
        MiniPick.run_action(rhs)
  ...
  return MiniExtra.pickers.colorschemes(
    ...
    vim.tbl_deep_extend("keep", opts or {}, {
      ...
      mappings = {
        ...
        scroll_up = "<PageUp>",
        move_down_alt = move_remap("<C-n>", "<Down>"),
        move_up_alt = move_remap("<C-p>", "<Up>"),
        move_start_alt = move_remap("<C-g>", "move_start"),
        scroll_down_alt = move_remap("<C-f>", "scroll_down"),
        scroll_up_alt = move_remap("<C-b>", "scroll_up"),
      },
    })
  )
end
local pick_remap = function(lhs, rhs)
  return {
    char = lhs,
    func = function()
      MiniPick.run_action(rhs)
...
MiniPick.setup({
  ...
  -- },
  mappings = {
    choose_marked_2 = pick_remap("<C-Enter>", "choose_marked"),
    choose_marked_3 = pick_remap("<C-q>", "choose_marked"),
    move_down_2 = pick_remap("<C-j>", "move_down"),
    move_up_2 = pick_remap("<C-k>", "move_up"),
    scroll_up_2 = pick_remap("<C-d>", "scroll_up"),
    toggle_preview_2 = pick_remap("<M-p>", "toggle_preview"),
    reveal_file = {
      char = "<S-Enter>",
      func = function()
